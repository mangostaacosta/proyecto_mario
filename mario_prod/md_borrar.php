...
******************************************************************************/
	...
	if (isset( $_GET['rango'] )){
		...
		echo "Falta el rango de antiguedad" ;
		die() ;
	}
	
	//se inveierte el areglo para que la ultima actualiización en la BD sea de los datos más recientes
	
	/* no hay que invertirla porque la priemra insertada se qued con el puesto, solo gana una poserior si la fecha es posterior
	$long = sizeof( $arr_list0 ) ;
	for ( $i = $long ; $i > 0 ; $i--){
		$arr_list[] = $arr_list0[$i-1] ;
	}
	*/
...
*/	
	...
	$psfecha = $arr_list[0]['fecha'] ;
	ActivarFechaBD_Rango( $psfecha ) ;
	...
	if ( is_array($arr_list) ){		//arreglar los textos ´para evitar errores de identificación y caracteres
		...
		foreach($arr_list2 as $key => $value){
			$conteo++ ;
			$fuente = 'metrocuadrado' ;		//OJO: reviasr cuando haya mas fuentes
			if ( isset( $value['tipo'] )){
				$tipo = $value['tipo'] ;
			}else{
			...
			// el idM2 no existe, puede que exista un papa gemeleado hay que buscarlo, pero se posible que toque insertar nuevo en t_inm
				...
				//NO: 3. Se busca en el listado de gemelos manuales, ESTÁ?
					...
					}//POSIBLE FINAL DEL NUEVO PARENTESIS
			...
			if ( $inserta_dw == 1 ){
			//con este IF esta tabla queda senciallamente como una ta_ asociacion de gemelos_exactos que se van automatizando ya que sólo los llena una vez, no guarda histórico
	...
	MsjE( "Inmuebles con Precio Modificados: {$arr_cont['prmodif']}" ) ;
	...
	// también asume que los campos de $arr_datos_inm viene ya prelimpiados para cotejar con datos de BD
	// creo que la mejor forma de utilizar esta función es asumir que los datos vienen en un arreglo asociativo con los  nombres de los campos de t_inm
		...
		if ( $g_conexion->returnedRows() > 0 ){
			...
			if ( $precio != $linea['precio'] ){
				...
				//el registro existe se necesita update				
					MsjE( "Precio para actualizar de {$linea['precio']} a $precio en $id_inmu el día $fecha<br> URL ANT: {$linea['url']} <br> URL NEW: $url" ) ;
					$sql = "
						UPDATE t_precios
						SET
						precio='$precio',
				...
				// la fecha no existe, hay que insertar
					MsjE( "Precio ha cambiado de {$linea['precio']} a $precio en $id_inmu el día $fecha<br> URL ANT: {$linea['url']} <br> URL NEW: $url" ) ;
					$sql = "
	...
	//Cambia los campos "mutables" del inmueble que no se relacionan con precio
		...
		// En ningún caso se actualiza la fecha, ya que esta marca el putno del último precio vigene, y acá no se analizó eso
	...
	//Esta fucnión se llama cunad se sabe que el precio cambió verifica si está manual
	...
	//OJO HAY QUE DEFINIR ESTA FUNCIÓN BIEN
	function TraeGemelo( $idM2 ){
		return 0 ;
	}
